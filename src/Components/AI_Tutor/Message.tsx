        ...
        try {
          const blob =
            file instanceof Blob
              ? file
              : new Blob([(file as any).data ? atob((file as any).data) : ""], {
                  type: file.type,
                });
    ...
    >
      ...
      <div
        className={`rounded-xl max-w-3xl w-full md:w-auto ${
          isUser ? "p-2 bg-blue-500 text-white" : "bg-white text-black"
        }`}
      >
        {/* Attachments */}
        ...
        {(displayedContent || content) && (
          <ReactMarkdown
            ...
            components={{
              ...
              h2: ({ node, ...props }) => (
                <h2 className="text-xl font-semibold mt-4 mb-2 text-black" {...props} />
              ),
              h3: ({ node, ...props }) => (
                <h3 className="text-lg font-semibold mt-3 mb-2 text-black" {...props} />
              ),
              p: ({ node, ...props }) => (
                <p className="leading-relaxed my-2 text-black" {...props} />
              ),
              a: ({ node, ...props }) => (
                <a
              ...
              blockquote: ({ node, ...props }) => (
                <blockquote
                  className="border-l-4 border-gray-300 pl-4 ml-2 my-4 text-gray-600 italic bg-gray-100 rounded"
                  {...props}
                />
              ),
              ul: ({ node, ...props }) => (
                <ul className="list-disc list-inside ml-4 my-2 text-black" {...props} />
              ),
              ol: ({ node, ...props }) => (
                <ol className="list-decimal list-inside ml-4 my-2 text-black" {...props} />
              ),
              li: ({ node, ...props }) => <li className="my-1 text-black" {...props} />,
              table: ({ node, ...props }) => (
                <table
                  className="table-auto border-collapse my-4 w-full text-sm text-left text-black"
                  {...props}
                />
              ),
              thead: ({ node, ...props }) => (
              ...
              th: ({ node, ...props }) => (
                <th className="px-4 py-2 font-semibold text-black" {...props} />
              ),
              td: ({ node, ...props }) => (
              ...
              code({ inline, className, children }: any) {
                ...
                const lang = match?.[1] || detectLanguage(codeString);

                if (!inline) {
                  return (
                    <div className="relative my-4 rounded-md overflow-hidden text-sm sm:text-xs border border-gray-300 bg-gray-100">
                      <div className="px-4 py-1 text-xs font-mono text-gray-700 bg-gray-200 border-b border-gray-300 capitalize">
                        {lang}
                      </div>
                ...
                return (
                  <code className="bg-gray-200 text-pink-600 px-1.5 py-0.5 rounded text-sm sm:text-xs">
                    {children}
                  </code>
                );
              },
